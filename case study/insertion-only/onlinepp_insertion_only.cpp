...
vector<int> cache_index;
vector<int> cache_alpha;
vector<int> cache_beta;
vector<vector<int> > cache_subgraph;
...
void get_instructions(char* instruction_address) {
	...
	vector<Instruction> deletion_instructions;
	...
	while (getline(infile, line)) {
		...
		if (inst.instruction_type == INSERT) {
			inst.instruction_type = DELETE;
			deletion_instructions.push_back(inst);
		}
	}

	vector<Instruction> result;
	result.reserve(deletion_instructions.size() + instructions.size());
	result.insert(result.end(), deletion_instructions.begin(), deletion_instructions.end());
	result.insert(result.end(), instructions.begin(), instructions.end());
	instructions = move(result);
...
void analyze_instruction_timings() {
	...
	int start = 0;
	while (start < n && instructions[start].instruction_type == DELETE) {
		++start;
	}

	vector<double> waiting_times;
	waiting_times.reserve(n - start);
	...
	for (int i = start; i < n; ++i) {
		...
		if (inst.instruction_type != QUERY) continue;
	...
	double sum = 0.0, max_wait = 0.0;
	for (double w : waiting_times) {
		sum += w;
		if (w > max_wait) max_wait = w;
	}
	double avg_wait = waiting_times.empty() ? 0.0 : sum / waiting_times.size();

	cout << fixed << setprecision(6);
	cout << "Analyzed " << waiting_times.size() << " INSERT/QUERY instructions\n";
	cout << "Average turnaround time: " << avg_wait << " s\n";
	cout << "Max turnaround time:     " << max_wait << " s\n";
}
...
int main(int argc, char** argv) {
	...
	int cache_size = 10;
	cache_index.resize(cache_size);
	cache_alpha.resize(cache_size);
	cache_beta.resize(cache_size);
	cache_subgraph.resize(cache_size);
	for (int c = 0; c < cache_size; c++) cache_index[c] = -c;

	G.initialize_orientation();

	int now_query_number = 0;
	for (int i = 0; i < instruction_number; i++) {
		...
		if (now_instruction.instruction_type == INSERT) {
			...
			for (int c = 0; c < cache_size; c++) {
				if (cache_index[c] == -c) continue;
				if (cache_alpha[c] == G.alpha && cache_beta[c] == G.beta) {
					G.D.clear();
					for (auto x : cache_subgraph[c]) G.D.insert(x);
					goto finish;
				}
				if (cache_subgraph[c].size() == 0 && cache_alpha[c] <= G.alpha && cache_beta[c] <= G.beta) {
					G.D.clear();
					goto finish;
				}
			}
			...
			for (int c = 0; c < cache_size; c++) {
				if (cache_subgraph[c].size() == 0) continue;
				if (cache_alpha[c] >= G.alpha && cache_beta[c] >= G.beta) {
					for (auto x : cache_subgraph[c])
						if (!G.C2.in[x]) G.C2.insert(x);
				}
			}
		...
		finish:
			for (int c = 0; c < cache_size; c++) {
				if (cache_index[c] == now_query_number - cache_size) {
					cache_index[c] = now_query_number, cache_alpha[c] = G.alpha, cache_beta[c] = G.beta;
					cache_subgraph[c].clear();
					for (int i = 0; i < G.D.size; i++) cache_subgraph[c].push_back(G.D.nodes[i]);
				}
			}
			now_query_number++;
		}
