...
void analyze_instruction_timings() {
	...
	for (int i = start; i < n; i++) {
		if (instructions[i].instruction_type == QUERY) {
			query_num++;
			query_sum += waiting_times[i - start];
			query_max = max(query_max, waiting_times[i - start]);
		}
		else if (instructions[i].instruction_type == INSERT || instructions[i].instruction_type == DELETE) {
			update_num++;
			update_sum += waiting_times[i - start];
			update_max = max(update_max, waiting_times[i - start]);
		}
	}
