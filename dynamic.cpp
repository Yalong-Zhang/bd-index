...
void generate_update_edges() {
	...
	for (auto x : s) update_edges.push_back(x);
}
...
int main(int argc, char** argv) {
	if (argc != 3) {
	argument_error:
		printf("Usage: ./main <dataset_address> <update_algorithm>\n");
	...
	generate_update_edges();

	timer.start();
	int len = update_edges.size();
	for (int i = 0; i < len; i++) {
		if (i % 10 == 0) printf("%d\r", i), fflush(stdout);
		if (algorithm_used == SPACE)
			G.delete_edge_space(update_edges[i]);
		else if (algorithm_used == TIME)
			G.delete_edge_time(update_edges[i]);
	}
	timer.end(); runtime = timer.time();
	printf("- %-20s: %lf\n", "Delete time", runtime);
	...
	for (int i = len - 1; i >= 0; i--) {
		if (i % 10 == 0) printf("%d\r", i), fflush(stdout);
		if (algorithm_used == SPACE)
			G.insert_edge_space(update_edges[i]);
		else if (algorithm_used == TIME)
			G.insert_edge_time(update_edges[i]);
	}
	timer.end(); runtime = timer.time();
	printf("- %-20s: %lf\n", "Insert time", runtime);
